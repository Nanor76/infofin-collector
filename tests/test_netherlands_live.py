...
def test_netherlands_afm_live_diagnose_discover_and_download(
    tmp_path: Path,
) -> None:
    settings = Settings(
        db_path=tmp_path / "netherlands-live.sqlite3",
        data_dir=tmp_path / "raw",
        http_timeout_seconds=45,
        http_retries=2,
        http_backoff_factor=0.5,
        user_agent="InfoFin Netherlands live integration test",
        max_download_bytes=250 * 1024 * 1024,
        amf_base_url="https://www.info-financiere.gouv.fr",
        amf_fallback_base_urls=(),
        amf_dataset="flux-amf-new-prod",
        amf_rows=100,
        netherlands_rate_limit_seconds=0.1,
    ...
    connector = NetherlandsAfmConnector(
        session=session,
    ...
    try:
        ...
        pdf = next(
            ...
            if candidate.metadata.get("filename", "")
            .casefold()
            .endswith(".pdf")
        )
    ...
    assert "netherlands" in first.path.parts
